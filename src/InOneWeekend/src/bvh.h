...
public:
  bvh_node(hittable_list list)
    ...
    // Theres a C++ subtelty here. This constructor (without span indices)
    // creates an implicit copy of the hittable list, which we will modify. The
    // lifetime of the copied list only extends untill this constructor exits.
    // Thats ok, because we only need to persist the resulting bounding volume
    // hierarchy
  }
  ...
  bool hit(cosnt ray &r, interval ray_t, hit_record &rec) const override {
    ...
    bool hit_right =
        right->hit(r, itnerval(ray_t.min, hit_left ? rec.t : ray_t.max), rec);
